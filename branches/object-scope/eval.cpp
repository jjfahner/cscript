...
{
  // Create global scope
  m_global = new GlobalScope(FilePos(), &GetGlobalScope());
...
Evaluator::EvalStatement(Ast* node)
{
  ...
  case compound_statement:
    if(node->m_a1)
    {
      AutoScope as(this, new Scope(node->m_pos, m_scope));
      EvalStatement(node->m_a1);
    }
    break;
...
Evaluator::EvalExpression(Ast* node)
{
  ...
  case literal_value:         return EvalLiteral(node);
...
Evaluator::EvalBinary(Ast* node)
{
  ...
  return m_scope->AddTemporary(result);
}
...
Evaluator::EvalPrefix(Ast* node)
{
  ...
  case op_predec: 
    ...
    return lhs;
  case op_negate: 
    return m_scope->AddTemporary(ValNeg(lhs));
  case op_not:    
    return m_scope->AddTemporary(ValNot(lhs));
  }
...
Evaluator::EvalPostfix(Ast* node)
{
  ...
  RValue& result = m_scope->AddTemporary(lhs);
...
Evaluator::EvalIndex(Ast* node)
{
  ...
  {
    // Create new variable
    val = new RWVariable(&lhs.GetObject(), Value());
  }
...
RValue&
Evaluator::EvalLiteral(Ast* node)
{
  return m_scope->AddTemporary(node->m_a1.GetValue());
}
...
Evaluator::EvalVarDecl(Ast* node)
{
  ...
  // Create variable
  m_scope->AddRWVariable(node->m_a1, value);
}

void
Evaluator::EvalFunDecl(Ast* node)
{
  m_scope->AddROVariable(node->m_a1, new ScriptFunction(node->m_a1, node));
}

RValue& 
Evaluator::EvalClosure(Ast* node)
{
  return m_scope->AddTemporary(new ScriptFunction(node->m_a1, node));
}

void 
Evaluator::EvalExternDecl(Ast* node)
{
  m_scope->AddROVariable(node->m_a1, new ExternFunction(node->m_a1, node));
}
...
Evaluator::EvalFunctionCall(Ast* node)
{
  ...
  try
  {
    return m_scope->AddTemporary(fun->Execute(this, args));
  }
  catch(return_exception const& e)
  {
    return m_scope->AddTemporary(e.m_value);
  }
}
...
Evaluator::EvalScriptCall(ScriptFunction* fun, Arguments& args)
{
  ...
  {
    // Create class scope
    cs.Set(new ClassScope(FilePos(), parentScope, args.GetObject()));
  ...
  AutoScope asf(this, new ClassScope(FilePos(), parentScope, fun));

  // Create argument scope
  AutoScope asa(this, new Scope(FilePos(), m_scope));
  ...
  {
    m_scope->AddRWVariable((*pi)->m_a1, args[index]);
  }

  // Create function execution scope
  AutoScope es(this, new Scope(FilePos(), m_scope));

  // Execute expression
  try
  {
    EvalStatement(fun->GetNode()->m_a3);
    return m_scope->AddTemporary(Value());
  }
  catch(return_exception const& e)
  {
    return m_scope->AddTemporary(e.m_value);
  ...
  for(;;)
  {
    ...
    if((*pi)->m_a2.GetNumber() == ptVariadic)
    {
      ...
      for(; ai != ae; ++ai)
      {
        ...
        // Append to list
        // TODO the new variable here is wrong!
        va->GetMembers()[va->GetMembers().size() - 1] = new RWVariable(0, val);
      }
...
Evaluator::EvalListLiteral(Ast* node)
{
  ...
  if(!node->m_a1.Empty())
  {
    ...
    while(child)
    {
      ...
      // 
      o.GetMembers()[key] = new RWVariable(0, element);
...
{
  // Outer scope
  AutoScope scope(this, new Scope(node->m_pos, m_scope));
...
Evaluator::EvalNewExpression(Ast* node)
{
  ...
  // Done
  return m_scope->AddTemporary(inst);
}
...
Evaluator::EvalMemberExpression(Ast* node)
{
  ...
  {
    throw script_exception(node, "Object has no member '" + node->m_a2->m_a1.GetString() + "'");
  }
...
Evaluator::EvalThisExpression(Ast* node)
{
  ...
  while(scope)
  {
    ...
    if(cs)
    {
      return m_scope->AddTemporary(cs->GetObject());
    }
  ...
  try
  {
    ...
    catch(user_exception const& e)
    {
      ...
      if(node->m_a2)
      {
        ...
        AutoScope scope(this, new Scope(node->m_pos, m_scope));
        m_scope->AddRWVariable(node->m_a2->m_a1, e.m_value);
